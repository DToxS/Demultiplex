...
protected:
    ...
    /// \brief Function to create a SeqType sequence
    ...
    ///               creation.
    virtual SeqType makeSequence(const ArgTypes&... args) = 0;
...
public:
    ...
    void writeSequences(const GroupIdType& group_id, SequencesType& group_seqs, bool flush=false)
    {
        ...
        if(verbose) std::cout << '\n' << "Group " << group_id << ": " << group_seqs.size() << " sequences" << '\n';
